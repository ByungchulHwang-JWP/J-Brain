...
const DeployActivateStage = ({ projectId, stage, summary }) => {
  ...
  const workCards = [
    {
      icon: UploadCloud,
      title: 'Export ZIP Import',
      description: '생성된 Service-Pack ZIP을 Runtime Pack Store에 반입하고 Loader 검증을 실행합니다.',
      action: 'Repository',
      panel: 'import',
      primary: true,
    },
    {
      icon: ShieldCheck,
      title: '승인/반려 처리',
      description: '검증된 Runtime Pack만 승인하고, 승인되지 않은 Pack은 활성화하지 않습니다.',
      action: '승인 관리',
      panel: 'approval',
    },
    {
      icon: PackageOpen,
      title: 'Active 전환',
      description: 'Approved Pack을 프로젝트의 Active Pack으로 전환합니다.',
      action: 'Active 관리',
      panel: 'active',
    },
    {
      icon: RotateCcw,
      title: 'Rollback 준비',
      description: '직전 정상 Pack으로 되돌릴 수 있도록 이전 Active Pack 이력을 유지합니다.',
      action: 'Rollback',
      panel: 'rollback',
    },
  ];
  ...
  const quickPanels = {
    import: {
      ...
      title: 'Export ZIP Import',
      description: 'Export된 Service-Pack ZIP을 고객 내부망 Runtime Pack Store에 반입하는 단계입니다.',
      items: [
    ...
    approval: {
      eyebrow: '6단계 배포 및 운영 개선',
      title: '승인/반려 처리',
      description: '검증된 Pack이라도 운영자가 승인해야 Active 전환 대상이 됩니다.',
      items: [
        { title: 'Approve', description: '검증 결과와 구성 내용을 확인한 Pack을 활성화 후보로 승인합니다.' },
        { title: 'Reject', description: '오류나 보완 필요가 있는 Pack은 반려하고 다음 Draft 보강으로 돌립니다.' },
        { title: '승인 이력', description: '승인자, 시각, 상태 변경 이력을 감사 로그로 남깁니다.' },
      ],
      secondaryAction: { label: '승인 관리 열기', onClick: () => navigate('/admin/packs/repository') },
    },
    active: {
      eyebrow: '6단계 배포 및 운영 개선',
      title: 'Active 전환',
      description: 'Approved Pack을 현재 프로젝트의 실제 Runtime 응답 기준 Pack으로 지정합니다.',
      items: [
        { title: 'Approved 상태 확인', description: '승인되지 않은 Pack은 활성화할 수 없습니다.' },
        { title: '직전 Pack 보관', description: '기존 Active Pack은 Rollback 후보로 보관합니다.' },
        { title: 'Runtime Resolver 준비', description: 'Runtime QA와 실제 위젯이 Active Pack 기준으로 동작하도록 연결합니다.' },
      ],
      secondaryAction: { label: 'Active 관리 열기', onClick: () => navigate('/admin/packs/repository') },
    },
    rollback: {
      eyebrow: '6단계 배포 및 운영 개선',
      title: 'Rollback 준비',
      description: '신규 Active Pack에 문제가 생겼을 때 직전 정상 Pack으로 되돌릴 수 있어야 합니다.',
      items: [
        { title: '이전 Pack 후보 확인', description: '직전 Active Pack 정보가 보관되어 있는지 확인합니다.' },
        { title: 'Rollback 실행', description: '필요 시 이전 Pack을 다시 Active 상태로 전환합니다.' },
        { title: '운영 이력 추적', description: 'Rollback 사유와 결과를 감사 로그에 남깁니다.' },
      ],
      secondaryAction: { label: 'Rollback 관리 열기', onClick: () => navigate('/admin/packs/repository') },
    },
  };

  const readiness = [
    { label: 'Export ZIP', done: exports.length > 0, meta: `${exports.length || summary.metrics.export_count || 0}건` },
    { label: 'Runtime Store', done: runtimePacks.length > 0, meta: `${runtimePacks.length || summary.metrics.runtime_pack_count || 0}건` },
    { label: 'Approved Pack', done: approvedPacks.length > 0, meta: `${approvedPacks.length}건` },
    ...
    <section className="workflow-deploy-stage">
      <div className="workflow-deploy-hero panel">
        <div>
          ...
          <h3>Runtime Pack Import 및 Active 전환</h3>
          <p>
            검증/승인된 Pack을 Runtime Pack Store에 Import하고 Active 버전으로 전환합니다.
            승인되지 않은 Pack은 활성화할 수 없으며, 직전 정상 Pack으로 Rollback할 수 있어야 합니다.
          </p>
        </div>
        <button className="btn-primary" type="button" onClick={() => setActivePanel('import')}>
          <PackageOpen size={16} /> Pack Repository 열기
        </button>
      </div>
      ...
      <div className="workflow-knowledge-metrics">
        ...
        <div className="panel workflow-status-card"><span>Runtime Store</span><strong>{runtimePacks.length || summary.metrics.runtime_pack_count || 0}건</strong><small>Import된 Pack</small></div>
        <div className="panel workflow-status-card"><span>Active Pack</span><strong>{activePack ? '있음' : '없음'}</strong><small>{activePack ? `${activePack.pack_id} v${activePack.pack_version}` : '활성화 대기'}</small></div>
      ...
      <div className="workflow-knowledge-grid">
        <div className="panel workflow-stage-worklist">
          <div className="workflow-board-head">
            <div>
              <h3>배포/활성화 작업</h3>
              <p>Export ZIP 반입부터 Active 전환까지 운영자가 처리해야 하는 작업입니다.</p>
            </div>
        ...
        <aside className="panel workflow-status-card">
          ...
          <div className="workflow-inline-note">
            {stage?.locked_reason || 'Active Pack이 준비되면 같은 6단계 안에서 실사용 로그와 미응답 개선 흐름을 관리합니다.'}
          </div>
        </aside>
      </div>
      ...
      <div className="workflow-knowledge-bottom">
        <div className="panel workflow-table-card">
          <div className="workflow-board-head">
            <div>
              ...
              <p>Import된 Pack의 승인/활성화 상태입니다.</p>
            </div>
            <FileArchive size={20} />
          </div>
          <table>
            <thead><tr><th>Pack</th><th>Status</th><th>승인</th><th>Source Export</th><th>Imported</th></tr></thead>
            <tbody>
              {loading ? (
                <tr><td colSpan="5">Runtime Pack 정보를 불러오는 중입니다.</td></tr>
              ) : runtimeRows.length === 0 ? (
                <tr><td colSpan="5">Import된 Runtime Pack이 없습니다.</td></tr>
                ...
                return (
                  <tr key={pack.import_id || `${pack.pack_id}-${pack.pack_version}`}>
                    <td><div className="name">{pack.pack_id} v{pack.pack_version} {isActive && <span className="badge active">ACTIVE</span>}</div><div className="meta">{pack.import_id || '-'}</div></td>
        ...
        <div className="panel workflow-stage-guide">
          ...
          <div className="workflow-guide-steps">
            ...
            <div><strong>{approvedPacks.length}</strong><span>승인/활성 후보 Pack</span></div>
      ...
      <div className="panel workflow-table-card">
        <div className="workflow-board-head">
          <div>
            <h3>최근 배포 감사 로그</h3>
            <p>Import, Validation, Approve, Activate, Rollback 작업 이력입니다.</p>
          </div>
          <History size={20} />
        </div>
        <table>
